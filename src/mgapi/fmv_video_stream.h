 ...
 * `abytes` = audio bytes per unit (the producer reads past them; FMV_AUDIO
 * handles audio). `nframes` from the header. `dma` is the transfer engine
 * (HOST→PSRAM frame fills). `depth` = ring depth, power of two ≥ 2 (use 4
 * for ~3 frames of lookahead). Returns a StreamHandle or
 * STREAM_HANDLE_INVALID; the caller still owns `fd` and `dma`. */
