...
void _start(void) {
    ...
    /* Sweep on by default. v2.30.14 fixed the whole-screen-black
     * flicker root cause (NMI's HIRQ re-arm was gated on frame_ready;
     * now runs every NMI). START toggles sweep off for manual D-pad
     * adjustment. */
    bool    sweep_on    = true;
    uint8_t top_lb      = 0;
    uint8_t bot_lb      = 0;
