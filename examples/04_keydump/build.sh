#!/bin/bash
# 04_keydump/build.sh — build the host and guest for this example.
#
# Usage:
...
if have_guest_cc; then
    ...
    if [ -f "$BUILD_DIR/guest.elf" ]; then
        ...
        echo "04_keydump: WARNING — $GUEST_CC not found and no guest.elf exists"
        echo "04_keydump:   install gcc-riscv64-unknown-elf or copy a prebuilt"
        echo "04_keydump:   guest.elf into $BUILD_DIR"
    fi
fi

echo "04_keydump: built. To run:"
echo "    $BUILD_DIR/host"

# Optional run.
if [ "${1:-}" = "run" ]; then
    echo
    echo "04_keydump: ===== running ====="
    "$BUILD_DIR/host"
fi
